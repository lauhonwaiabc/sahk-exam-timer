...
window.OsceTimer = (() => {
  ...
  function renderStationMode(sessionIdx, label) {
    ...
    if (isIsolated && window.scoringIsolateStation) {
      var t = smc.querySelector('.osce-box[aria-label="Station ' + window.scoringIsolateStation + ' details"]');
  ...
  function applyIsolate() {
    ...
    if (foundSt) {
      var smc = document.getElementById('stationModeContainer');
      if (smc) {
        var boxes = smc.querySelectorAll('.osce-box');
        boxes.forEach(function(b) { b.classList.remove('highlight'); });
        var target = smc.querySelector('.osce-box[aria-label="Station ' + foundSt + ' details"]');
  ...
  function renderTransitContent(sessionIdx) {
    ...
    const nextIdx = C.getNextExamSession(sessionIdx, osceData);
