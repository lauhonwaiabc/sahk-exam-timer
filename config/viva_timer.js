...
window.VivaTimer = (() => {
  ...
  function renderTableMode(sessionIdx, label) {
    ...
    if (isIsolated) {
      document.querySelectorAll('.viva-box.highlight, .candidate-cell.highlight').forEach(function(b) {
        b.classList.remove('highlight');
        b.setAttribute('data-iso', '');
      });
      if (window.scoringIsolateStation) {
        tableMode.querySelectorAll('.viva-box[aria-label="Table ' + window.scoringIsolateStation + ' details"]').forEach(function(t) {
  ...
  function renderBothSessions(prevIdx, nextIdx) {
    ...
    if (isIsolated) {
      ...
      if (window.scoringIsolateStation) {
        container.querySelectorAll('.viva-box[aria-label="Table ' + window.scoringIsolateStation + ' details"]').forEach(function(t) {
          t.setAttribute('data-iso', '');
        });
      }
    }
  ...
  function renderTransitContent(sessionIdx) {
    ...
    const nextIdx = C.isExamSession(sessionIdx, vivaData) ? sessionIdx : null;
  ...
  function updateVisibility() {
    ...
    C.updateIsolateScale('tableModeContainer');
  ...
  function applyIsolate() {
    ...
    document.querySelectorAll('.viva-box, .candidate-cell').forEach(function(b) { b.removeAttribute('data-iso'); });
    highlightBoxes(val);
    document.querySelectorAll('.viva-box.highlight, .candidate-cell.highlight').forEach(function(b) {
      b.classList.remove('highlight');
      b.setAttribute('data-iso', '');
